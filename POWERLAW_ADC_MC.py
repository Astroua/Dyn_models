...
def log_adc(p, R, vobs, sobs, evobs, esobs, Vinf_in, Rc_in, incl_in, beta_in,sig0_in,ksig_in):

    Vinf, Rc, incl, beta, sig0, ksig = p[0], p[1], p[2], p[3], p[4], p[5]
...
# %&%&%&%&%&%&%&%&%&%&%&
gals=['488','772','4102','5678','3949','4030','2964','864','4254','3423','2805']

Vinf_all=[220.52,182.47,156.75,186.25,115.48,220.49,222.74,152.79,92.12,127.03,22.63] 
Rc_all= [7.9,5.76,4.11,8.84,15.42,8.87,17.50,12.22,12.63,39.49,2.63]
incl_all=[42,54,55,61,55,44,57,41,29,32,41]
beta_all=[0.65,0.65,0.65,0.65,0.55,0.55,0.55,0.40,0.40,0.25,0.15]
sig0_all=[240.79,154.93,145.79,111.73,61.77,108.46,92.98,78.43,76.10,56.74,67.42]
ksig_all=[-4.42,-0.38,-3.54,-1.08,1.77,-0.03,-0.21,0.13,0.19,0.76,-0.38]
...
for j in range(len(gals)):
    ...
    # Set the walkers
    ndim, nwalkers = 6,60 #60
    p0 = np.zeros((nwalkers,ndim))
    p0[:,0] = np.random.randn(nwalkers)*10+Vinf_in
    p0[:,1] = np.random.randn(nwalkers)*1+Rc_in
    p0[:,2] = np.random.randn(nwalkers)*5+incl_in
    p0[:,3] = np.random.randn(nwalkers)*0.5+beta_in
    ...
    sampler = emcee.EnsembleSampler(nwalkers, ndim, log_adc,
              args=[R,vobs,sobs,evobs,esobs, Vinf_in, Rc_in, incl_in, beta_in,sig0_in,ksig_in], threads=12)
    ...
    fig = plt.figure(figsize=(10,6))
    triangle.corner(sampler.flatchain, labels=["$Vinf$", "$Rc$",
                                           "$ideg$", "$beta_z$", "$sig_0$", "$k_{\sigma}$"])
    fig.gca().annotate("NGC"+gal, xy=(0.5, 1.0), xycoords="figure fraction",
                      xytext=(0, -5), textcoords="offset points",
                      ha="center", va="top")
    plt.savefig("figures/triangles/NGC"+gal+"_ADC_triangle.pdf")
    plt.close()
    #--------------------------------------------
    bins_adc = 50
    ...
    plt.title(r'Distribution of $V_{inf}$')
    plt.subplot(2,3,2)
    patches = plt.hist(sampler.flatchain[:,1],bins=bins_adc)
    plt.title(r'Distribution of $R_{c}$')
    plt.subplot(2,3,3)
    ...
    plt.subplot(2,3,6)
    patches = plt.hist(sampler.flatchain[:,5],bins=bins_adc)
    plt.title(r'Distribution of $k_{\sigma}$')

    plt.savefig("figures/NGC"+gal+"/Vcirc_ADC_histo.pdf")
    plt.close()
    #---------------------------------------------
    Vinf_dist = sampler.flatchain[:,0]
    Rc_dist = sampler.flatchain[:,1]
    incl_dist = sampler.flatchain[:,2]
    betaz_dist = sampler.flatchain[:,3]
    sig0_dist = sampler.flatchain[:,4]
    ksig_dist = sampler.flatchain[:,5]

    Vinf_med = np.median(sampler.flatchain[:,0])
    Rc_med = np.median(sampler.flatchain[:,1])
    incl_med = np.median(sampler.flatchain[:,2])
    betaz_med = np.median(sampler.flatchain[:,3])
    sig0_med = np.median(sampler.flatchain[:,4])
    ksig_med = np.median(sampler.flatchain[:,5])

    Vinf_plus = np.percentile(sampler.flatchain[:,0], 75)- np.median(sampler.flatchain[:,0])
    Rc_plus = np.percentile(sampler.flatchain[:,1], 75)- np.median(sampler.flatchain[:,1])
    incl_plus = np.percentile(sampler.flatchain[:,2], 75)- np.median(sampler.flatchain[:,2])
    betaz_plus = np.percentile(sampler.flatchain[:,3], 75)- np.median(sampler.flatchain[:,3])
    sig0_plus = np.percentile(sampler.flatchain[:,4], 75)- np.median(sampler.flatchain[:,4])
    ksig_plus = np.percentile(sampler.flatchain[:,5], 75)- np.median(sampler.flatchain[:,5])

    Vinf_minus = np.median(sampler.flatchain[:,0]) - np.percentile(sampler.flatchain[:,0], 25)
    Rc_minus = np.median(sampler.flatchain[:,1]) - np.percentile(sampler.flatchain[:,1], 25)
    incl_minus = np.median(sampler.flatchain[:,2]) - np.percentile(sampler.flatchain[:,2], 25)
    betaz_minus = np.median(sampler.flatchain[:,3]) - np.percentile(sampler.flatchain[:,3], 25)
    sig0_minus = np.median(sampler.flatchain[:,4]) - np.percentile(sampler.flatchain[:,4], 25)
    ksig_minus = np.median(sampler.flatchain[:,5]) - np.percentile(sampler.flatchain[:,5], 25)

    #-----------------------------------------------
    # Array for the medians
    filename='data_output/Tables/Table_NGC_'+gal+'.txt'
    medians = [Vinf_med, Rc_med, incl_med,betaz_med,sig0_med,ksig_med]

    # Array for the upper percentiles
    ups = [Vinf_plus, Rc_plus, incl_plus,betaz_plus,sig0_plus,ksig_plus]

    # Array for the lower percentiles
    lws = [Vinf_minus, Rc_minus, incl_minus,betaz_minus,sig0_minus,ksig_minus]

    t = Table([medians, ups, lws], names = ('medians(Vinf, Rc, incl,betaz,sig0,ksig)','ups','lws'))
    ...
    print 'Vinf: ', np.median(sampler.flatchain[:,0]), \
    ...
    print 'Rc: ', np.median(sampler.flatchain[:,1]), \
    ...
    print 'betaz: ', np.median(sampler.flatchain[:,3]), \
    ...
    print 'sig0: ', np.median(sampler.flatchain[:,4]), \
    ...
    print 'ksig: ', np.median(sampler.flatchain[:,5]), \
        '+', np.percentile(sampler.flatchain[:,5], 75) - np.median(sampler.flatchain[:,5]),\
        '-', np.median(sampler.flatchain[:,5]) - np.percentile(sampler.flatchain[:,5], 25)             



    Vphi_mod = np.zeros([len(R),len(sampler.flatchain[:,0])])
    alphar_mod = np.zeros([len(R),len(sampler.flatchain[:,0])])
    ...
    for j in range(len(R)):
        ...
        sigr_obs[j,:] = sobs[j]/np.sqrt(1 - \
                                        ...
                                        0.5*(Rc_dist**2/(R[j]**2+Rc_dist**2)-1)*np.sin(math.pi/180*incl_dist)**2)
        sobs_fin[j,:] = (sig0_dist + ksig_dist*R[j])*np.sqrt(1 - \
                                        betaz_dist*np.cos(math.pi/180*incl_dist)**2 + \
                                        0.5*(Rc_dist**2/(R[j]**2+Rc_dist**2)-1)*np.sin(math.pi/180*incl_dist)**2)

        vobs_fin[j,:] = Vinf_dist*R[j]*np.sin(math.pi/180.*incl_dist)/np.sqrt(Rc_dist**2+R[j]**2)
    ...
    plt.plot(R,np.median(Vc_mod, axis = 1), 'bo', label='ADC')
    ...
    plt.errorbar(R,np.median(Vc_mod, axis = 1), yerr = (eVctop, eVcbot), color = 'b' )
